...
func TestErrorUnmarshal(t *testing.T) {
	...
	addr, _ := url.Parse("https://loclahost/test")
...
func TestFullRequestCycle(t *testing.T) {
	...
	c.Transport = &mockHTTP{
		impl: func(r *http.Request) (*http.Response, error) {
			if r.URL.String() != "https://loclahost/test/vim25/8.0.0.1/VirtualMachine/vm-42/AcquireMksTicket" {
			...
			if strings.Trim(reqBody, " \t\n\r") != "{}" {
				t.Errorf("Unexpected body value: '%v'", reqBody)
			}
			if t.Failed() {
				t.FailNow()
			}
