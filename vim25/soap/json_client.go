...
func (c *Client) invoke(ctx context.Context, this types.ManagedObjectReference, method string, params interface{}, res HasFault) error {
	buffer := bytes.Buffer{}
	if params != nil {
		marshaller := types.NewGovmomiEncoder(&buffer)
		marshaller.Encode(params)
	}
...
func (c *Client) responseUnmarshaler(result interface{}) func(resp *http.Response) error {
	return func(resp *http.Response) error {
		...
		if resp.StatusCode == 500 {
			...
			dec := types.NewGovmomiDecoder(bytes.NewReader(bodyBytes))
			...
			f := &Fault{
				XMLName: xml.Name{
					// TODO: We use the namespace of the client i.e. the namespace of the method.
					// This is potentially incorrect - Can we use the vim25 errors as if they are
					// declared in vsan namespace e.g. "vsan:InvalidLogin"? Do we need the "urn"
					// prefix i.e. "urn:vim25"?
					Space: c.Namespace,
					// TODO: I am unsure how the InvalidLoginFault is formed.
					// The code below is wild guess.
					Local: reflect.TypeOf(serverErr).Name() + "Fault",
				},
				String: faultStringStruct.Msg,
		...
		dec := types.NewGovmomiDecoder(resp.Body)
		e := dec.Decode(result)
		if e != nil {
			return e
		}
...
func (c *Client) getPathForName(this types.ManagedObjectReference, name string) string {
	...
	ns := c.Namespace
	if strings.HasPrefix(ns, urn_prefix) {
		ns = ns[len(urn_prefix):]
	}
