...
func TestOptionValueSerialization(t *testing.T) {
	...
	options := []struct {
		...
		// 	binding: OptionValue{Key: "option1", Value: "test"},
		// },
		// array is weird. Do we want to unmarshal this as
		...
		// if SOAP does it then I guess back compat is a big reason.
		{
			name: "binary", // []byte
			wire: `{"_typeName": "OptionValue","key": "option1",
				"value": {"_typeName": "ArrayOfString","_value": ["Hello"]}}`,
			binding: OptionValue{Key: "option1",
				Value: ArrayOfString{String: []string{"Hello"}}},
		},
		{
		...
		// We process these in the serialization logic i.e. discover or
		// prepend the "ArrayOf" prefix.
		// This makes to go bindings incompatible between types that have
		// "ArrayOf*" struct defined and those that don't. We can perhaps remove
		// all of the "ArrayOf*" types pending similar fix in XML.
		{
			name: "array of enum",
			wire: `{
...
func TestFirstToUpper(t *testing.T) {
	tests := []struct {
		...
		{upper: "Алфа", lower: "алфа"},
		{upper: "İ123alpha", lower: "i123alpha"},
		{upper: "I123alpha", lower: "ı123alpha"},
	}
	for _, test := range tests {
		t.Run("ToUpper "+test.lower, func(t *testing.T) {
			assert.Equal(t, test.upper, firstToUpper(test.lower))
			assert.Equal(t, test.upper, firstToUpper(test.upper))
		})
		t.Run("ToLower "+test.upper, func(t *testing.T) {
			assert.Equal(t, test.lower, firstToLower(test.upper))
			assert.Equal(t, test.lower, firstToLower(test.lower))
		})
	}
}
